...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@vvv1ti"))
...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@vvv2ti"))
    except BaseException:
        pass
...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@vvv3ti"))
    except BaseException:
        pass  
...
async def gcast(event):
    septhon = event.pattern_match.group(1)
    if sython:
        msg = sython
    elif event.is_reply:
